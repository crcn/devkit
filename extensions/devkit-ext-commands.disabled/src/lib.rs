    ...
    fn menu_items(&self, ctx: &AppContext) -> Vec<MenuItem> {
        ...
        for (pkg_name, pkg_config) in &ctx.config.packages {
            for (cmd_name, cmd_entry) in &pkg_config.cmd {
                ...
                match cmd_entry {
                    devkit_core::CmdEntry::Simple(_) => {
                        entry.push((pkg_name.clone(), None));
                    }
                    devkit_core::CmdEntry::Full(config) => {
                        // Add default variant
                        entry.push((pkg_name.clone(), None));
        ...
        for (cmd_name, packages) in all_commands {
            ...
            // Create menu items
            for (variant, pkgs) in by_variant {
...
fn execute_command(ctx: &AppContext, cmd_name: &str, variant: Option<&str>) -> devkit_core::Result<()> {
    let opts = devkit_tasks::CmdOptions {
        packages: vec![],
        parallel: false,
    ...
    run_cmd(ctx, cmd_name, &opts).map_err(Into::into)?;
    Ok(())
}
